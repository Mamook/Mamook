...
echo '<nav id="navbar" class="nav1">',
	'<ol>',
		...
		'<li class="list-nav-1', ((strstr(FULL_URL, 'MyAccount/')!== FALSE) ? '' : ' hover'), Document::addHereClass(SECURE_URL.'MyAccount/', FALSE, FALSE), '">',
			'<a href="', SECURE_URL ,'MyAccount/profile.php" title="My Account">My Account</a>',
			'<ul class="nav-2">',
				'<li class="list-nav-2', Document::addHereClass(SECURE_URL.'MyAccount/profile.php', FALSE, FALSE), '">',
					'<a href="', SECURE_URL, 'MyAccount/profile.php" title="Update Profile">Update Profile</a>',
				'</li>',
	...
	if($login->checkAccess(ALL_BRANCH_USERS)===TRUE)
	{
		echo '<li class="list-nav-1', (((strstr(FULL_URL, 'ManageContent/')!==FALSE)) ? '' : ' hover'), Document::addHereClass(ADMIN_URL.'ManageContent/', FALSE, FALSE),'">',
			...
			require TEMPLATES.'content_nav.php';
		echo '</li>';
	}
