<?php /* framework/command_line/Media/VideoUpload.php */
...
if($passed_data['YouTube']==='post_youtube')
{
	...
	$commandline_obj->runScript(Utility::locateFile(COMMAND_LINE.'Media'.DS.'YouTubeUpload.php'), $passed_data);
}
...
if(!empty($passed_data['FileName']))
{
	...
	if(isset($passed_data['NewVideo']) && $passed_data['NewVideo']==TRUE)
	{
		if(empty($passed_data['ImageID']))
		{
			...
			$db->query('UPDATE `'.DBPREFIX.'videos` SET `image`='.$db->escape($image_id).' WHERE `id`='.$db->quote($passed_data['ID']).' LIMIT 1');
		}
	}
}
*/