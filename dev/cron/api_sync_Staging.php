 ...
 * 00      00      *       *       *       /opt/local/bin/php <Full Path to Cron Folder>/cron/api_sync_Staging.php
 *
...
foreach($all_db_videos as $db_video)
{
	...
	$video_list_response=$yt->listVideos('snippet', array('id'=>$video_api_decoded->youtube_id));
	# If empty, then the video does not exist on YouTube.
	if(empty($video_list_response['items']))
	{
		# Delete the video.
		$delete_video=$db->query('DELETE FROM `'.DBPREFIX.'videos` WHERE `id` = '.$db_video->id.' LIMIT 1');
	}
}
...
foreach($all_playlists as $playlist_row)
{
	...
	$playlist_items=$yt->PlaylistItems($playlist_api_decoded->YouTube->playlist_id);
	...
	if(!empty($db_videos))
	{
		...
		if(!empty($filtered_array))
		{
			...
			foreach($filtered_array as $filtered)
			{
				...
				{
					# Create a video list request.
					$video_list_response=$yt->listVideos('snippet,status', array('id'=>$filtered['videoId']));
