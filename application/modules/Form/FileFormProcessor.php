...
class FileFormProcessor extends FormProcessor
{
	...
	public function processFile($data, $max_size=104857600)
	{
		try
		{
			...
			require_once MODULES.'Form'.DS.'FileFormPopulator.php';

			# Remove any un-needed CMS session data. (This needs to happen before populatFileForm is called but AFTER the Populator has been included so that the getCurrentURL method will be available.)
			...
			$publisher_name=$file->getPublisher();
			...
			if(array_key_exists('_submit_check', $_POST) && (isset($_POST['file']) && ($_POST['file']=='Add File' OR $_POST['file']=='Update')))
			{
				...
				{
					# Set an error.
					$fv->setErrors('You must select at least one category for this file.');
				}
				$u_file=$_FILES['file'];
					...
					if($_GET['delete']==='yes')
					{
						...
						if(array_key_exists('_submit_check', $_POST) && isset($_POST['do']) && (isset($_POST['delete_file']) && ($_POST['delete_file']==='delete')))
						{
							...
							if($record_retrieved===TRUE)
							{
								...
								# Check if this user has access to delete this image.
								if($access===TRUE)
								{
	 ...
	 * @return	string
	 */
	private function processFileSelect()
	{
	 ...
	 * @access	private
	 */
	private function redirectFile($file_name, $action)
	{
		try
		{
			# Set the Document instance to a variable.
			...
			$file->getThisFile($file_name, FALSE);
			...
			if(isset($_SESSION['form']['post']) OR isset($_SESSION['form']['product']))
			{
