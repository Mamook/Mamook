...
class AccountFormProcessor extends FormProcessor
{
	...
	public function processAccount($data, $max_size=7340032)
	{
		try
		{
			...
			require_once MODULES.'Form'.DS.'AccountFormPopulator.php';

			# Remove any un-needed CMS session data.
			# This needs to happen before populatAccountForm is called but AFTER the Populator has been included so that the getCurrentURL method will be available.
			//$this->loseSessionData('account_desc');
...
/*
			...
			$this->processAccountBack();
*/
			//$this->processAccountSelect();
			$this->processAccountFocus();

			# Get the Staff object from the AccountFormPopulator object and set it to a variable for use in this method.
			...
			$address=$staff_obj->getAddress();
			# Set the user's address2 to a variable.
			$address2=$staff_obj->getAddress2();
			# Set the user's affiliation to a variable.
			$affiliation=$staff_obj->getAffiliation();
			# Set the user's archive to a variable.
			$archive=$staff_obj->getArchive();
			# Set the user's bio to a variable.
			$bio=$staff_obj->getBio();
			# Set the user's city to a variable.
			$city=$staff_obj->getCity();
			# Set the user's country to a variable.
			$country=$staff_obj->getCountry();
			# Set the user's credentials to a variable.
			...
			if(WP_INSTALLED===TRUE)
			{
				...
				$nickname=$staff_obj->getNickname();
			}
			# Set the user's phone to a variable.
			$phone=$staff_obj->getPhone();
			# Set the user's position to a variable.
			...
			if(array_key_exists('_submit_check', $_POST) && (isset($_POST['account']) && ($_POST['account']=='Add User' OR $_POST['account']=='Update')))
			{
				...
				# Check if there is a record in the `staff` table associated with this User.
				if($staff_obj->getStaffID()!==NULL)
				{
				...
				if($empty_display===FALSE)
				{
					...
					$unique_display=$staff_obj->checkUnique('display', $display_name, ' AND `ID` != '.$db->quote($id));
				...
				if($empty_email===FALSE)
				{
					...
					if($real_email===TRUE)
					{
						...
						$unique_email=$staff_obj->checkUnique('email', $email, ' AND `ID` != '.$db->quote($id));
...
*/
					...
					if(!empty($id))
					{
						try
						{
							...
							$update_user=$staff_obj->updateUser($where, $field_value);
							...
							if($staff_obj->getStaffID()!==NULL)
							{
								...
								$update_person=$staff_obj->updatePerson($where, $field_value);
							}
	 ...
	 * processAccountBack
	 *
	 * Processes a submitted form indicating that the User should be sent back to the form that sent from.
	 *
	 * @access	private
	 */
	private function processAccountBack()
	{
		try
		{
			# Create an array of possible indexes. These are forms that can send the user to get account info.
			$indexes=array(
				'account'
			);
			# Set the resource value.
			$resource='account_desc';
...
*/
	 ...
	 * processAccountFocus
	 *
	 * Processes a submitted account form for position descriptions.
	 *
	 * @access	private
	 * @return	string
	 */
	private function processAccountFocus()
	{
		try
		{
			# Bring the alert-title variable into scope.
			...
			# Reset the form if the "reset" button was submitted.
			$this->processReset('account_desc', 'account_desc');
			...
			# Check if the form has been submitted.
			if(array_key_exists('_submit_check', $_POST) && (isset($_POST['account_desc']) && ($_POST['account_desc']=='Update')))
			{
				# Create a session that holds all the POST data (it will be destroyed if it is not needed.)
	...
	private function redirectAccount($account_name, $action)
	{
		try
		{
			...
			$staff_obj->findUserData($account_name);
			...
			if(isset($_SESSION['form']['account']) || isset($_SESSION['form']['account_desc']))
			{
				...
				{
					# Set the form's name as "account_desc".
					$origin_form='account_desc';
					# Set the post session account index name.
					$account_index='Account';
					# Set the content session account name.
		...
		try
		{
			...
			$_SESSION['form']['account']=
				array(
					...
					'Credentials'=>$staff_obj->getCredentials(),
					'CV'=>$staff_obj->getCV(),
					'DisplayName'=>$staff_obj->getDisplayName(),
					'Email'=>$staff_obj->getEmail(),
					'FirstName'=>$staff_obj->getFirstName(),
					'FormURL'=>$form_url,
					'ID'=>$staff_obj->getID(),
					'Img'=>$staff_obj->getImg(),
					'ImgTitle'=>$staff_obj->getImgTitle(),
					'Interests'=>$staff_obj->getInterests(),
					'LastName'=>$staff_obj->getLastName(),
					'StaffMiddleName'=>$staff_obj->getStaffMiddleName(),
